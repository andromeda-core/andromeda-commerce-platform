...
export default function create({ folders: initialFolders = [] }) {
    ...
    const [imageSlots, setImageSlots] = useState([{ id: 0, file: null, preview: null }]);
    ...
    const handleFileRemove = (index) => {
        const updated = imageSlots.filter((_, i) => i !== index);
        setImageSlots(updated.length > 0 ? updated : [{ id: Date.now(), file: null, preview: null }]);
    };

    const addSlot = () => {
        setImageSlots([...imageSlots, { id: Date.now(), file: null, preview: null }]);
    };

    const allSlotsFilled = imageSlots.length > 0 && imageSlots.every((s) => s.file !== null);
    ...
    const handleCreateFolder = async (e) => {
        ...
        try {
            const res = await axios.post(route('dashboard.internal-product-images.store-folder'), {
    ...
    const handleSubmit = (e) => {
        ...
        const files = imageSlots.map((s) => s.file).filter(Boolean);
        ...
        if (files.length === 0) {
            setSubmitErrors({ images: 'Please select at least one image.' });
            return;
        }

        setSubmitting(true);
        router.post(
            route('dashboard.internal-product-images.store'),
            { folder: selectedFolder, images: files },
            {
                forceFormData: true,
    ...
    return (
        <>
            <AuthenticatedLayout>
                ...
                <BreadCrumb
                    header={'Upload Product Images'}
                    parent={'Product Images'}
                    parent_link={route('dashboard.internal-product-images.index')}
                    child={'Upload'}
                />
                    ...
                    Content={
                        <>
                            <div className="my-3 flex flex-wrap justify-end">
                                <LinkButton
                                    Text={'Back To Product Images'}
                                    URL={route('dashboard.internal-product-images.index')}
                                    Icon={
                                        <svg
                                    ...
                                    Content={
                                        <>
                                            ...
                                            <div className="mb-4">
                                                ...
                                                <div className={`grid grid-cols-1 gap-4${imageSlots.length > 1 ? ' sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4' : ''}`}>
                                                    {imageSlots.map((slot, index) => (
                                                        <CustomFileUploader
                                                            ...
                                                            uploadButtonText="Click to select an image"
                                            ...
                                            {/* Add More + Submit */}
                                            <div className="flex flex-wrap items-center gap-3">
                                                {allSlotsFilled && imageSlots.length < 20 && (
                                                    <button
                                                        type="button"
                                                        onClick={addSlot}
                                                        className="inline-flex items-center gap-1.5 rounded-lg border border-gray-300 bg-transparent px-3 py-2.5 text-sm font-medium text-gray-700 hover:bg-gray-50 dark:border-gray-700 dark:text-gray-300 dark:hover:bg-gray-800"
                                                ...
                                                <PrimaryButton
                                                    Text={`Upload ${imageSlots.filter((s) => s.file).length > 0 ? imageSlots.filter((s) => s.file).length + ' Image(s)' : 'Images'}`}
                                                    Type={'submit'}
                                                    CustomClass={'w-[220px]'}
                                                    Disabled={
                                                        submitting ||
                                                        !selectedFolder ||
                                                        imageSlots.every((s) => s.file === null)
                                                    }
                                                    Spinner={submitting}
                                                    Icon={
                                                        <svg
