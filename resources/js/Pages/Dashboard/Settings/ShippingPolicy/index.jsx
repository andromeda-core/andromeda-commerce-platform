...
export default function index({ shipping_policies }) {
    ...
    const toggleStatus = (id) => {
        put(route('dashboard.settings.shipping-policy-settings.toggle-status', id));
    };

    const [columns, setColumns] = useState([]);
    useEffect(() => {
        const columns = [
            { key: 'name', label: 'Shipping Policy Name' },
            { key: 'language.name', label: 'Language' },
            {
                label: 'Shipping Policy Status',
                render: (item) => {
                    if (item.is_active === 1) {
                        return (
                            <>
    ...
    return (
        <>
            <AuthenticatedLayout>
                ...
                <BreadCrumb
                    header={'Settings - Shipping Policy Settings'}
                    parent={'Settings'}
                    ...
                    Content={
                        <>
                            <div className="flex flex-wrap justify-end gap-4 my-3">
                                <LinkButton
                                    Text={'Create Shipping Policy'}
                                    URL={route('dashboard.settings.shipping-policy-settings.create')}
                                    Icon={
                                        <svg
                            ...
                            <Table
                                ...
                                BulkDeleteRoute={
                                    'dashboard.settings.shipping-policy-settings.destroybyselection'
                                }
                                SingleDeleteRoute={
                                    'dashboard.settings.shipping-policy-settings.destroy'
                                }
                                EditRoute={'dashboard.settings.shipping-policy-settings.edit'}
                                SearchRoute={'dashboard.settings.shipping-policy-settings.index'}
                                Search={false}
