...
export default function edit({ shipping_policy, languages }) {
    // Edit Data Form Data
    const { data, setData, put, processing, errors } = useForm({
        name: shipping_policy?.name || '',
        dpo_name: shipping_policy?.dpo_name || '',
        dpo_email: shipping_policy?.dpo_email || '',
        dpo_phone: shipping_policy?.dpo_phone || '',
        dpo_address: shipping_policy?.dpo_address || '',
        company_name: shipping_policy?.company_name || '',
        country: shipping_policy?.country || '',
        state: shipping_policy?.state || '',
        content: shipping_policy?.content ?? [
            { title: '', content: '' }
        ],
        language_id: shipping_policy?.language_id || '',
    });
    ...
    const submit = (e) => {
        e.preventDefault();
        put(route('dashboard.settings.shipping-policy-settings.update', shipping_policy?.id));
    };

    return (
        <>
            <AuthenticatedLayout>
                <Head title="Settings - Shipping Policy Settings" />

                <BreadCrumb
                    header={'Settings - Edit Shipping Policy'}
                    parent={'Shipping Policy Settings'}
                    parent_link={route('dashboard.settings.shipping-policy-settings.index')}
                    child={'Shipping Policy Settings'}
                />
                                    ...
                                    Content={
                                        <>
                                            <div className="grid grid-cols-1 gap-4 mb-10 md:grid-cols-2">
                                                <Input
                                                    InputName={'Shipping Policy Name'}
                                                    Error={errors.name}
                                                    Value={data.name}
                                                    Action={(e) =>
                                                        setData(
                                                    ...
                                                    Placeholder={'Enter Shipping Policy Name'}
                                                    Id={'name'}
                                                    Name={'name'}
                                                    Type={'text'}
                                                    Required={true}
                                                ...
                                                <SelectInput
                                                    InputName={'Shipping Policy Language'}
                                                    ...
                                                    Placeholder={'Select Shipping Policy Language'}
