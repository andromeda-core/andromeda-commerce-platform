...
export default function index({ now_payment_settings }) {
    ...
    const toggleStatus = (id) => {
        put(route('dashboard.settings.now-payment-settings.toggle-status', id));
    };

    const [columns, setColumns] = useState([]);
    useEffect(() => {
        const columns = [
            { key: 'now_payment_api_key', label: 'NOWPayment API Key' },
            { key: 'now_payment_public_key', label: 'NOWPayment Public Key' },
            { key: 'now_payment_baseurl', label: 'NOWPayment Base URL' },

            {
                label: 'NOWPayment Status',
                render: (item) => {
                    if (item.is_active === 1) {
                        return (
                            <>
    ...
    return (
        <>
            <AuthenticatedLayout>
                ...
                <BreadCrumb
                    header={'Settings - NOWPayment Settings'}
                    parent={'Settings'}
                        ...
                        <>
                            <div className="my-3 flex flex-wrap justify-end gap-4">
                                <LinkButton
                                    ...
                                    URL={route('dashboard.settings.now-payment-settings.create')}
                                    Icon={
                                        <svg
                            ...
                            <Table
                                ...
                                BulkDeleteRoute={
                                    'dashboard.settings.now-payment-settings.destroybyselection'
                                }
                                SingleDeleteRoute={
                                    'dashboard.settings.now-payment-settings.destroy'
                                }
                                EditRoute={'dashboard.settings.now-payment-settings.edit'}
                                SearchRoute={'dashboard.settings.now-payment-settings.index'}
                                Search={false}
                                DefaultSearchInput={false}
                                items={now_payment_settings}
                                props={props}
                                columns={columns}
                            />
                        </>
