...
export default function index({ return_policies }) {
    ...
    const toggleStatus = (id) => {
        put(route('dashboard.settings.return-policy-settings.toggle-status', id));
    };

    const [columns, setColumns] = useState([]);
    useEffect(() => {
        const columns = [
            { key: 'name', label: 'Return Policy Name' },
            { key: 'language.name', label: 'Language' },
            {
                label: 'Return Policy Status',
                render: (item) => {
                    if (item.is_active === 1) {
                        return (
                            <>
    ...
    return (
        <>
            <AuthenticatedLayout>
                ...
                <BreadCrumb
                    header={'Settings - Return Policy Settings'}
                    parent={'Settings'}
                    ...
                    Content={
                        <>
                            <div className="flex flex-wrap justify-end gap-4 my-3">
                                <LinkButton
                                    Text={'Create Return Policy'}
                                    URL={route('dashboard.settings.return-policy-settings.create')}
                                    Icon={
                                        <svg
                            ...
                            <Table
                                ...
                                BulkDeleteRoute={
                                    'dashboard.settings.return-policy-settings.destroybyselection'
                                }
                                SingleDeleteRoute={
                                    'dashboard.settings.return-policy-settings.destroy'
                                }
                                EditRoute={'dashboard.settings.return-policy-settings.edit'}
                                SearchRoute={'dashboard.settings.return-policy-settings.index'}
                                Search={false}
