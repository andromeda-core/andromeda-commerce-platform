...
export default function index({ terms_of_services }) {
    ...
    const toggleStatus = (id) => {
        put(route('dashboard.settings.terms-of-service-settings.toggle-status', id));
    };

    const [columns, setColumns] = useState([]);
    useEffect(() => {
        const columns = [
            { key: 'name', label: 'Terms Of Service Name' },
            { key: 'language.name', label: 'Language' },
            {
                label: 'Terms Of Service Status',
                render: (item) => {
                    if (item.is_active === 1) {
                        return (
                            <>
    ...
    return (
        <>
            <AuthenticatedLayout>
                ...
                <BreadCrumb
                    header={'Settings - Terms Of Service Settings'}
                    parent={'Settings'}
                        ...
                        <>
                            <div className="flex flex-wrap justify-end gap-4 my-3">
                                <LinkButton
                                    ...
                                    URL={route('dashboard.settings.terms-of-service-settings.create')}
                                    Icon={
                                        <svg
                            ...
                            <Table
                                ...
                                BulkDeleteRoute={
                                    'dashboard.settings.terms-of-service-settings.destroybyselection'
                                }
                                SingleDeleteRoute={
                                    'dashboard.settings.terms-of-service-settings.destroy'
                                }
                                EditRoute={'dashboard.settings.terms-of-service-settings.edit'}
                                SearchRoute={'dashboard.settings.terms-of-service-settings.index'}
                                Search={false}
