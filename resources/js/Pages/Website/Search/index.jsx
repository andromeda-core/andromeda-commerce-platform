...
const ResultItem = memo(
    ({ item, onCopyLink, generateURL, generateSmartphoneURL, activeView, __, currency }) => {
...
const index = ({
    ...
    const [state, setState] = useState({
        ...
        searchHistories:
            Object.values(all_search_histories).length > 0
                ? all_search_histories
                : search_histories || [],
    ...
    const updateScrollButtons = useCallback(() => {
        if (scrollContainerRef.current) {
            ...
            if (
                newCanScrollLeft !== state.canScrollLeft ||
                newCanScrollRight !== state.canScrollRight
            ) {
                updateState({
        ...
        try {
            ...
            if (res.data.status) {
                setState((prev) => {
                    ...
                    const newUnique = (res.data.results || []).filter(
    ...
    const fetchSearchHistoryResults = useCallback(
        async (historyId) => {
    ...
    const handleTabChange = useCallback(
        ...
        [
            ...
            hasPerformedSearch,
            fetchHistoryResults,
            fetchSearchHistoryResults,
        ],
    );

    // Fetch more results with optimization
    const fetchMoreResults = useCallback(async () => {
        const currentTabNextPageUrl =
            state.activeTab === 'all'
                ? state.historyNextPageUrl
                : typeof state.activeTab === 'number'
                  ? state.historyResultsCache[state.activeTab]?.nextPageUrl
                  : null;
            ...
            setState((prev) => {
                if (prev.activeTab === 'all') {
                    const existingKeys = new Set(
                        prev.historyResults.map((item) => `${item.type}-${item.id}`),
                    );
                    ...
                    const existingData = prev.historyResultsCache[prev.activeTab] || {
                        results: [],
                        nextPageUrl: null,
                    };
                    const existingKeys = new Set(
        ...
        async (id) => {
            try {
                ...
                if (result.isConfirmed) {
                    ...
                    if (res.data.status) {
                        setState((prev) => {
                            const updatedHistories = prev.searchHistories.filter(
                                (h) => h.id !== id,
                            );
        ...
        () => [
            {
                ...
                label: 'All',
                count:
                    hasPerformedSearch() && currentResults.length === 0
                        ? 0
    ...
    return (
        <MainLayout>
            ...
            <div className="mt-0 min-h-screen pb-20 lg:mt-5">
                <div className="mx-auto w-full max-w-6xl overflow-x-hidden px-4 text-black dark:text-main-text-dark sm:px-8">
                    <GlobalSearch
                        floors={floors}
                    ...
                    <div className="mb-4 mt-6 w-full">
                        <div className="relative grid w-full grid-cols-1 overflow-hidden">
                            <div className="relative flex w-full items-center">
                                ...
                                <button
                                    onClick={scrollLeft}
                                    className={`absolute left-0 z-20 flex flex-shrink-0 items-center justify-center rounded-full bg-surface-2-light p-2 transition-all duration-200 hover:scale-110 dark:bg-surface-3-dark md:flex ${state.canScrollLeft ? 'opacity-100' : 'pointer-events-none opacity-0'}`}
                                    style={{ left: '0px' }}
                                >
                                    <svg
                                        xmlns="http://www.w3.org/2000/svg"
                                ...
                                <div
                                    ...
                                    className="flex w-full flex-nowrap items-center gap-3 overflow-x-auto scroll-auto scrollbar-none"
                                    style={{
                                ...
                                >
                                    ...
                                    {tabs.map((tab) => (
                                        <button
                                            ...
                                            className={`relative flex flex-shrink-0 items-center gap-2 whitespace-nowrap rounded-full px-4 py-2 text-sm font-medium ${
                                    ...
                                    {state.searchHistories.map((history) => (
                                        ...
                                        >
                                            ...
                                            <span
                                                className={`text-xs ${state.activeTab === history.id ? 'font-semibold text-main-text-dark dark:text-main-text-light' : 'text-main-text-light dark:text-main-text-dark'}`}
                                            >
                                                (
                                                {state.historyResultsCache[history.id]?.results
                                                    .length || history.results_count}
                                                )
                                            </span>
                                            <div
                                                onClick={(e) => {
                                                ...
                                                className={`flex-shrink-0 rounded-full p-0.5 hover:bg-main-text-light/10 dark:hover:bg-main-text-dark/10`}
                                            >
                                                <svg
                                                    xmlns="http://www.w3.org/2000/svg"
                                                    fill="none"
                                ...
                                <button
                                    onClick={scrollRight}
                                    className={`absolute right-0 z-20 flex flex-shrink-0 items-center justify-center rounded-full bg-surface-2-light p-2 transition-all duration-200 hover:scale-110 dark:bg-surface-3-dark md:flex ${state.canScrollRight ? 'opacity-100' : 'pointer-events-none opacity-0'}`}
                                >
                                    <svg
                                        xmlns="http://www.w3.org/2000/svg"
                                        fill="none"
                    ...
                    {/* Match Types Row */}
                    {matchTypes.length > 0 && (
                        <div className="flex min-w-0 flex-col gap-3 py-2 sm:flex-row sm:items-center sm:justify-between">
                            ...
                            <div className="flex items-center gap-3">
                                {state.activeTab !== 'all' &&
                                    (() => {
                                        ...
                                        return (
                                            <div
                                                className="flex items-center gap-2 rounded-full bg-surface-1-light px-3 py-1.5 text-xs font-medium text-sub-text-light dark:bg-surface-1-dark dark:text-sub-text-dark"
                                                role="status"
                                ...
                                {matchTypes.map((mt) => (
                                    <div
                                        ...
                                        onClick={() =>
                                            updateState({
                                                activeMatchType:
                                                    state.activeMatchType === mt ? null : mt,
                                            })
                                        }
                                        className={`cursor-pointer py-1 text-xs text-main-text-light transition-colors dark:text-main-text-dark`}
                                        aria-pressed={state.activeMatchType === mt}
                                    >
                                        <div className="flex items-center">
                                            <span
                                                className={`${state.activeMatchType === mt ? 'font-semibold' : ''} text-sm`}
                                            >
                                                {mt.replace(/_/g, ' ')} (
                                                {matchTypeGroups[mt].length})
                                            </span>

                                            {state.activeMatchType === mt && (
                                                <button
                                                    onClick={() =>
                                                        updateState({ activeMatchType: null })
                                                    }
                                                    className="mx-2 text-sm text-main-text-light transition-colors dark:text-main-text-dark"
                        ...
                        !(state.searchHistoryLoading && typeof state.activeTab === 'number') && (
                            <div className="py-16 text-center" role="status">
                                <svg
                                    ...
                                    strokeWidth={1.5}
                                    stroke="currentColor"
                                    className="mx-auto h-16 w-16 text-main-text-light dark:text-main-text-dark"
                                    aria-hidden="true"
                                >
                                    <path
                                        strokeLinecap="round"
                                ...
                                <p className="mt-4 line-clamp-2 whitespace-normal break-words break-all text-lg font-medium text-main-text-light dark:text-main-text-dark">
                                    {getNoResultsMessage()}
                                </p>
