...
const PackageVerificationRecorder = memo(({
    ...
    const handleScannedCode = useCallback(async (text) => {
        if (isVerifiedRef.current || isScanningRef.current) return;
        isScanningRef.current = true;

        const trimmed = text?.trim();
        if (!trimmed) { isScanningRef.current = false; return; }
        ...
        try {
            ...
            if (onVerify) {
                ...
                isSuccess = data.status === true || data.status === 1 || data.status === 'success';
            ...
            if (isSuccess) {
                ...
                stopRecording();
                const msg = responseMsg || `Verification successful. CODE ${trimmed} matched.`;
    ...
    useEffect(() => {
        ...
        let cleanup = () => { };
        runScanner().then(fn => { if (fn) cleanup = fn; });
                ...
                <div className="relative z-10 w-full max-w-2xl text-gray-900 bg-white shadow-2xl rounded-2xl dark:bg-deepcharcoal dark:text-white/80">
                    ...
                    <div className="flex items-center justify-between px-6 pt-5 pb-4 border-b dark:border-white/10">
                        <h3 className="text-base font-semibold">
                            {desktopScanActive ? 'Hold Barcode in the Scan Box' : 'Package Verification Recorder'}
                        </h3>
                        <div className="flex items-center gap-2">
                            ...
                            {!recordedUrl && !desktopScanActive && (
                                <button
                                    ...
                                    className={`flex items-center justify-center rounded-lg w-8 h-8 transition-colors ${torchOn
                    ...
                    <div className="p-5">
                        ...
                        {verificationStatus === 'success' && (
                            <div className="flex items-center gap-3 p-3 mb-4 border border-green-200 rounded-lg bg-green-50 dark:bg-green-900/20 dark:border-green-800">
                                <div className="flex items-center justify-center flex-shrink-0 bg-green-100 rounded-full w-7 h-7 dark:bg-green-900/40">
                        ...
                        {verificationStatus === 'mismatch' && (
                            <div className="flex items-center gap-3 p-3 mb-4 border border-red-200 rounded-lg bg-red-50 dark:bg-red-900/20 dark:border-red-800">
                                <div className="flex items-center justify-center flex-shrink-0 bg-red-100 rounded-full w-7 h-7 dark:bg-red-900/40">
                                    <svg className="w-4 h-4 text-red-600 dark:text-red-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M6 18L18 6M6 6l12 12" />
                                    </svg>
                                </div>
                                <p className="text-sm font-medium text-red-800 dark:text-red-300">
                                    {verificationMessage || 'CODE mismatch. Video discarded. Recording restarted.'}
                                </p>
                            </div>
                        )}
                        {verificationStatus === 'scan_error' && (
                            <div className="flex items-center gap-3 p-3 mb-4 border border-orange-200 rounded-lg bg-orange-50 dark:bg-orange-900/20 dark:border-orange-800">
                                <div className="flex items-center justify-center flex-shrink-0 bg-orange-100 rounded-full w-7 h-7 dark:bg-orange-900/40">
                                    <svg className="w-4 h-4 text-orange-600 dark:text-orange-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" />
                                    </svg>
                                </div>
                                <p className="text-sm font-medium text-orange-800 dark:text-orange-300">
                                    {verificationMessage || 'Scan failed. Recording resumed, try again.'}
                                </p>
                            </div>
                        )}
                        ...
                        >
                            ...
                            {desktopScanActive && !recordedUrl && (
                                ...
                                >
                                    ...
                                    <div
                                        ...
                                        style={{ width: '65%', height: '56px', boxShadow: 'none' }}
                                    >
                                        {/* Corner accents */}
                                        <div className="absolute -top-0.5 -left-0.5 w-4 h-4 border-t-4 border-l-4 border-green-400 rounded-tl" />
                                        <div className="absolute -top-0.5 -right-0.5 w-4 h-4 border-t-4 border-r-4 border-green-400 rounded-tr" />
                                        <div className="absolute -bottom-0.5 -left-0.5 w-4 h-4 border-b-4 border-l-4 border-green-400 rounded-bl" />
                                        <div className="absolute -bottom-0.5 -right-0.5 w-4 h-4 border-b-4 border-r-4 border-green-400 rounded-br" />
                                        {/* Animated scan line */}
                                        <div className="absolute top-1/2 left-2 right-2 h-0.5 -translate-y-1/2 bg-green-400 opacity-80 animate-pulse" />
                                    </div>
                                    {/* Hint text */}
                                    <p className="absolute bottom-4 left-1/2 -translate-x-1/2 text-white text-xs px-3 py-1.5 rounded-full bg-black/70 whitespace-nowrap">
                            ...
                            {desktopScanActive && !recordedUrl && (
                                <div className="absolute flex items-center gap-2 px-3 py-1 text-xs font-semibold text-white bg-green-600 rounded-full top-3 left-3">
                            ...
                            {!isRecording && desktopScanActive && (
                                <div className="absolute flex items-center gap-2 px-3 py-1 text-xs font-semibold text-white rounded-full bg-amber-500 top-3 right-3">
                                    ⏸ Recording Paused
                                </div>
                            )}
                            ...
                            {isRecording && !desktopScanActive && (
                                <div className="absolute flex items-center gap-2 px-3 py-1 text-xs text-white -translate-x-1/2 rounded-full bottom-3 left-1/2 bg-black/60 whitespace-nowrap">
                            ...
                            {!recordedUrl ? (
                                <>
                                    ...
                                    {desktopScanActive && (
                                        <button
                                            ...
                                            className="flex items-center gap-2 px-5 py-2 text-sm font-medium text-white transition-transform bg-gray-500 rounded-lg hover:bg-gray-600 active:scale-95"
                                        >
                                        ...
                                        <>
                                            ...
                                            {isRecording && (
                                                <button
                                                    ...
                                                    className="flex items-center gap-2 px-5 py-2 text-sm font-medium text-white transition-transform rounded-lg bg-violet-600 hover:bg-violet-700 active:scale-95"
                                                >
                                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                                            ...
                                            {isRecording && (
                                                <button
                                                    ...
                                                    className="px-5 py-2 text-sm font-medium text-white transition-transform bg-red-600 rounded-lg hover:bg-red-700 active:scale-95"
                                                >
                                                    Stop Recording
                                                </button>
                                            )}
                                            ...
                                            {!isReady && !isRecording && !cameraError && (
                                                <p className="text-sm text-gray-400 dark:text-white/40 animate-pulse">
                                                    Preparing camera...
                                                </p>
                                            )}
                                ...
                                <>
                                    ...
                                    <button
                                        ...
                                        title={!isVerifiedRef.current ? 'Scan a barcode to verify the order first' : ''}
